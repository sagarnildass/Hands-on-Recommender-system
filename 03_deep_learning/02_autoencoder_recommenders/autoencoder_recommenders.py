        ...
        def autoencoder_loss(reconstructed, original, encoded=None):
            ...
            if self.use_sparse and encoded is not None:
                ...
                sparsity_loss = torch.mean(
                    ...
                    * torch.log(
                        self.sparsity_target / (torch.mean(encoded, dim=0) + 1e-8)
                    )
                    + (1 - self.sparsity_target)
                    * torch.log(
                        (1 - self.sparsity_target)
                        / (1 - torch.mean(encoded, dim=0) + 1e-8)
                    )
                )
...
def preprocess_data_for_autoencoder(
    ratings_df, test_size=0.2, val_size=0.1, random_state=42
):
    """
    ...
    train_users, test_users = train_test_split(
        user_ids, test_size=test_size, random_state=random_state
    )
    train_users, val_users = train_test_split(
        train_users, test_size=val_size, random_state=random_state
    )
...
def create_autoencoder_data_loaders(
    train_matrix, val_matrix, test_matrix, batch_size=32, noise_factor=0.1
):
    """
...
def plot_training_history(
    train_losses, val_losses, save_path="autoencoder_training_history.png"
):
    """
...
def run_autoencoder_demo():
    ...
    (
        ...
        test_matrix,
        full_matrix,
    ) = preprocess_data_for_autoencoder(ratings_subset)
    ...
    for model_name, model in models.items():
        ...
        plot_training_history(
            train_losses,
            val_losses,
            f'{model_name.lower().replace(" ", "_")}_history.png',
        )
    ...
    for i, (item_idx, score) in enumerate(recommendations, 1):
        ...
        movie_title = (
            movies[movies["movieId"] == movie_id]["title"].iloc[0]
