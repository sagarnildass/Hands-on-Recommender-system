...
class NeuralCollaborativeFiltering(nn.Module):
    ...
    def __init__(
        self, num_users, num_items, embedding_dim=32, layers=[64, 32, 16], dropout=0.1
    ):
        ...
        for layer_size in layers:
            self.mlp_layers.extend(
                [nn.Linear(input_dim, layer_size), nn.ReLU(), nn.Dropout(dropout)]
            )
    ...
    def train_model(
        ...
        for epoch in tqdm(
            range(num_epochs), desc="Training epochs", disable=not verbose
        ):
            # Training phase
            self.train()
            train_loss = 0.0
...
def compare_with_baseline(ncf_results, baseline_results):
    ...
    comparison = {"NCF": ncf_results, "Baseline": baseline_results}
...
def run_ncf_demo():
    ...
    # Preprocess data
    train_data, val_data, test_data, user_encoder, item_encoder = preprocess_data(
        ratings_subset
    )
    ...
    user_embeddings = model.get_embeddings(
        user_ids=[user_encoder.transform([test_user])[0]], device=device
    )
    item_embeddings = model.get_embeddings(
        item_ids=[item_encoder.transform([recommendations[0][0]])[0]], device=device
    )
