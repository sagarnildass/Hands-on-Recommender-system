...
class AttentionBasedRecommender(nn.Module):
    ...
    """
    def __init__(self, num_users, num_items, embed_dim=64, num_heads=8, 
    ...
    def forward(self, user_ids, item_sequences, attention_mask=None):
        ...
        # Get embeddings
        user_embeds = self.user_embedding(user_ids).unsqueeze(1)  # [batch_size, 1, embed_dim]
        item_embeds = self.item_embedding(item_sequences)  # [batch_size, seq_len, embed_dim]
...
class SequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(full_seq) > self.max_seq_len:
            ...
            full_seq = full_seq + [0] * (self.max_seq_len - len(full_seq))  # Pad with zeros
        
        # Create attention mask
        attention_mask = [1] * min(len(full_seq), self.max_seq_len) + [0] * max(0, self.max_seq_len - len(full_seq))
...
def train_attention_model(model, train_loader, val_loader, num_epochs=30, 
                         learning_rate=0.001, device='mps', verbose=True, patience=5):
    """
...
def visualize_attention_weights(model, user_id, item_sequence, item_names, device='mps'):
    """
    ...
    mask_tensor = torch.LongTensor([[1] * len(item_sequence) + [0] * (model.max_seq_len - len(item_sequence))]).to(device)
    ...
    for i, attention_weights in enumerate(attention_weights_list):
        ...
        avg_attention = attention_weights[0].mean(dim=0).cpu().numpy()  # [seq_len, seq_len]
        
        # Plot heatmap
        sns.heatmap(avg_attention[:len(item_sequence), :len(item_sequence)], 
...
def generate_attention_recommendations(model, user_id, user_sequence, item_to_idx, idx_to_item, 
                                     movies_df, n_recommendations=10, device='mps'):
    """
    ...
    seq_indices = [item_to_idx.get(item, 0) for item in user_sequence if item in item_to_idx]
    ...
    # Create attention mask
    attention_mask = [1] * min(len(user_sequence), model.max_seq_len) + [0] * max(0, model.max_seq_len - len(user_sequence))
    ...
    with torch.no_grad():
        ...
        for item_idx in tqdm(range(1, len(item_to_idx)), desc="Generating recommendations"):
            # Create sequence with target item
...
def run_attention_demo():
    ...
    encoded_sequences = []
    for user_id, history, target_item, target_rating in tqdm(sequences, desc="Encoding sequences"):
        if target_item in item_encoder.classes_:
            encoded_user = user_encoder.transform([user_id])[0]
            encoded_history = [item_encoder.transform([item])[0] for item in history if item in item_encoder.classes_]
            encoded_target = item_encoder.transform([target_item])[0]
            
            if len(encoded_history) > 0:
                encoded_sequences.append((encoded_user, encoded_history, encoded_target, target_rating))
    ...
    # Evaluate model
    rmse, mae, predictions, actuals = evaluate_attention_model(model, test_loader, device)
    print(f"\n Evaluation Results:")
    ...
    item_names = [movies[movies['movieId'] == idx_to_item[idx]]['title'].iloc[0] 
                 for idx in test_user_sequence if idx in idx_to_item]
    
    visualize_attention_weights(model, test_user_id, test_user_sequence, item_names, device)
