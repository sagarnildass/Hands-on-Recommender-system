...
class OptimizationMethods:
    ...
    def sgd_optimization(self, user_item_matrix, verbose=True):
        ...
        for epoch in tqdm(range(self.n_epochs), desc="Training epochs"):
            ...
            for user_idx, item_idx, rating in tqdm(ratings, desc="Processing ratings", leave=False):
                # Forward pass
    ...
    def sgd_momentum_optimization(self, user_item_matrix, momentum=0.9, verbose=True):
        ...
        for epoch in tqdm(range(self.n_epochs), desc="Training epochs"):
            ...
            for user_idx, item_idx, rating in ratings:
                ...
                # Update momentum
                U_momentum[user_idx] = momentum * U_momentum[user_idx] + self.learning_rate * U_grad
                V_momentum[item_idx] = momentum * V_momentum[item_idx] + self.learning_rate * V_grad
    ...
    def adaptive_sgd_optimization(self, user_item_matrix, beta1=0.9, beta2=0.999, epsilon=1e-8, verbose=True):
        """
        ...
        for epoch in tqdm(range(self.n_epochs), desc="Training epochs"):
            ...
            for user_idx, item_idx, rating in ratings:
                ...
                U[user_idx] += self.learning_rate * U_m_corrected / (np.sqrt(U_v_corrected) + epsilon)
                V[item_idx] += self.learning_rate * V_m_corrected / (np.sqrt(V_v_corrected) + epsilon)
    ...
    def evaluate_method(self, U, V, test_ratings, method_name):
        ...
        for _, row in tqdm(test_ratings.iterrows(), desc="Evaluating predictions", leave=False):
    ...
    def compare_optimization_methods(self, ratings, test_size=0.2):
        ...
        for i in tqdm(range(user_item_matrix.shape[0]), desc="Processing users"):
            for j in tqdm(range(user_item_matrix.shape[1]), desc="Processing items", leave=False):
                if user_item_matrix.iloc[i, j] > 0:
        ...
        for method_name, method_func in tqdm(methods, desc="Testing methods"):
            ...
            results[method_name] = {
                'metrics': metrics,
                'training_time': self.history[method_name.lower().replace(' + ', '_').replace(' ', '_')]['time'],
                'final_loss': self.history[method_name.lower().replace(' + ', '_').replace(' ', '_')]['losses'][-1]
            }
    ...
    def plot_comparison(self, results):
        ...
        # RMSE comparison
        axes[0].bar(methods, rmse_scores, color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4'])
        axes[0].set_title('RMSE Comparison')
        axes[0].set_ylabel('RMSE')
        axes[0].tick_params(axis='x', rotation=45)
        
        # Training time comparison
        axes[1].bar(methods, training_times, color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4'])
        axes[1].set_title('Training Time Comparison')
        axes[1].set_ylabel('Time (seconds)')
        axes[1].tick_params(axis='x', rotation=45)
        
        # Final loss comparison
        axes[2].bar(methods, final_losses, color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4'])
