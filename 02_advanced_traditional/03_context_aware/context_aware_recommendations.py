...
def create_context_features(ratings):
    ...
    ratings["day_category"] = pd.cut(
        ratings["day_of_week"], bins=[0, 5, 7], labels=["Weekday", "Weekend"]
    )
...
class ContextAwareRecommender:
    ...
    def _create_context_profiles(self, ratings):
        ...
        for user_id in tqdm(
            ratings["userId"].unique(), desc="Creating user context profiles"
        ):
    ...
    def _create_movie_context_preferences(self, ratings, movies):
        ...
        for movie_id in tqdm(
            movies["movieId"].unique(), desc="Creating movie context preferences"
        ):
    ...
    def get_context_aware_recommendations(self, user_id, context, n_recommendations=10):
        ...
        for movie_id in tqdm(
            self.movie_context_preferences, desc="Calculating context-aware scores"
        ):
            if (
                movie_id not in rated_movies
                and movie_id in self.user_item_matrix.columns
            ):
                ...
                for context_type, context_value in context.items():
                    if (
                        context_type in user_context_prefs
                        and context_type in movie_context_prefs
                    ):
                        if (
                            context_value in user_context_prefs[context_type]
                            and context_value in movie_context_prefs[context_type]
                        ):
                            user_pref = user_context_prefs[context_type][context_value]
                            movie_perf = movie_context_prefs[context_type][
                                context_value
                            ]
...
def analyze_context_patterns(ratings):
    ...
    activity_patterns = ratings.groupby("activity_context")["rating"].agg(
        ["mean", "count"]
    )
    ...
    print("\n🔄 Cross-Context Analysis:")
    cross_context = (
        ratings.groupby(["time_of_day", "day_category"])["rating"].mean().unstack()
    )
    print(cross_context)
