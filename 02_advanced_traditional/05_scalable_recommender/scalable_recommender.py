...
class ScalableRecommender:
    ...
    def _process_in_chunks(self, data: pd.DataFrame, process_func) -> List:
        ...
        for i in tqdm(range(0, len(data), self.chunk_size), desc="Processing chunks"):
            ...
            if self._get_memory_usage() > self.memory_limit_gb:
                print(
                    f"  Memory usage high ({self._get_memory_usage():.2f}GB), forcing garbage collection..."
                )
                gc.collect()
    ...
    def _build_user_item_matrix_chunked(self, ratings: pd.DataFrame) -> pd.DataFrame:
        ...
        matrix = pd.DataFrame(
            0, index=range(len(unique_users)), columns=range(len(unique_items))
        )
        ...
        print(
            f"✅ Matrix built: {final_matrix.shape[0]} users × {final_matrix.shape[1]} items"
        )
        return final_matrix

    def _compute_similarity_with_cache(
        self, matrix: pd.DataFrame, similarity_type: str = "cosine"
    ) -> np.ndarray:
        """
    ...
    def _compute_similarity_chunked(
        self, matrix: pd.DataFrame, similarity_type: str
    ) -> np.ndarray:
        """
    ...
    def incremental_update(self, new_ratings: pd.DataFrame):
        ...
        new_matrix = pd.DataFrame(
            0, index=range(len(self.user_to_idx)), columns=range(len(self.item_to_idx))
        )
    ...
    def fit(self, ratings: pd.DataFrame):
        ...
        self.user_similarity = self._compute_similarity_with_cache(
            self.user_item_matrix, "cosine"
        )

        # Compute item similarities with caching
        self.item_similarity = self._compute_similarity_with_cache(
            self.user_item_matrix.T, "cosine"
        )
    ...
    def recommend_for_user(
        self, user_id: int, n_recommendations: int = 10, method: str = "collaborative"
    ...
    def _collaborative_recommendations(
        self, user_idx: int, n_recommendations: int
    ) -> List[Tuple[int, float]]:
        """
    ...
    def evaluate(self, test_ratings: pd.DataFrame) -> Dict[str, float]:
        ...
        for _, row in tqdm(test_ratings.iterrows(), desc="Evaluating predictions"):
            ...
            if user_id in self.user_to_idx and item_id in self.item_to_idx:
                ...
                if len(item_ratings) > 0:
                    weighted_sum = sum(
                        r * s for r, s in zip(item_ratings, similarities)
                    )
                    similarity_sum = sum(similarities)
        ...
        results = {"rmse": rmse, "mae": mae, "n_predictions": len(predictions)}
