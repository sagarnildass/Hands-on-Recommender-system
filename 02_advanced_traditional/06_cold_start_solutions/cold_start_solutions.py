...
class ColdStartSolutions:
    ...
    def _create_user_clusters(self, ratings: pd.DataFrame) -> Dict[int, int]:
        ...
        for i, user_id in tqdm(
            enumerate(user_item_matrix.index), desc="Mapping users to clusters"
        ):
    ...
    def _create_item_clusters(self, tfidf_features: np.ndarray) -> Dict[int, int]:
        ...
        # Create mapping
        item_cluster_map = {}
        for idx, cluster_id in tqdm(
            enumerate(item_clusters), desc="Mapping items to clusters"
        ):
    ...
    def recommend_for_new_user(
        self, user_id: int, n_recommendations: int = 10, method: str = "popularity"
    ...
    def recommend_for_new_item(
        self, item_id: int, n_recommendations: int = 10
    ) -> List[Tuple[int, float]]:
        """
    ...
    def _popularity_recommendations(
        ...
        recommendations = []
        for item_id, score in tqdm(
            sorted_items[:n_recommendations],
            desc="Generating popularity recommendations",
        ):
            recommendations.append((item_id, score))

        return recommendations

    def _cluster_based_recommendations(
        ...
        cluster_items = []
        for item_id, cluster_id in tqdm(
            self.item_clusters.items(), desc="Finding cluster items"
        ):
    ...
    def _diversity_recommendations(
        self, n_recommendations: int
    ) -> List[Tuple[int, float]]:
        """
        ...
        for item_id, cluster_id in tqdm(
            self.item_clusters.items(), desc="Grouping items by cluster"
        ):
            popularity = self.popularity_scores.get(item_id, 0)
        ...
        for cluster_id in tqdm(
            range(self.n_clusters), desc="Selecting diverse recommendations"
        ):
    ...
    def _content_based_item_recommendations(
        self, item_id: int, n_recommendations: int
    ) -> List[Tuple[int, float]]:
        """
    ...
    def hybrid_cold_start_recommendations(
        self,
        user_id: int,
        ...
        cluster_recs = dict(
            self._cluster_based_recommendations(user_id, n_recommendations * 2)
        )
    ...
    def progressive_cold_start(
        self,
        user_id: int,
        initial_ratings: Optional[List[Tuple[int, float]]] = None,
        n_recommendations: int = 10,
    ) -> List[Tuple[int, float]]:
        """
        ...
        if initial_ratings is None or len(initial_ratings) == 0:
            ...
            print(
                f"   {len(initial_ratings)} initial ratings - using content-based approach"
            )
            return self._content_based_from_ratings(
                user_id, initial_ratings, n_recommendations
            )

    def _content_based_from_ratings(
        self, user_id: int, ratings: List[Tuple[int, float]], n_recommendations: int
    ) -> List[Tuple[int, float]]:
        """
    ...
    def evaluate_cold_start(
        self, test_users: List[int], test_items: List[int], movies: pd.DataFrame
    ) -> Dict[str, float]:
        """
        ...
        results = {
            ...
            "overall_score": (
                user_results["diversity_score"] + item_results["similarity_score"]
            )
            / 2,
        }
    ...
    def _evaluate_user_cold_start(
        self, test_users: List[int], movies: pd.DataFrame
    ) -> Dict[str, float]:
        """
        ...
        diversity_score = (
            unique_items / total_recommendations if total_recommendations > 0 else 0
        )
    ...
    def _evaluate_item_cold_start(self, test_items: List[int]) -> Dict[str, float]:
        ...
        return {
            ...
            "avg_recommendations_per_item": len(similarity_scores) / 10
            if similarity_scores
            else 0,
        }

    def _calculate_genre_diversity(
        self, item_ids: List[int], movies: pd.DataFrame
    ) -> float:
        """
    ...
    def compare_cold_start_methods(
        ...
        methods = {
            ...
            "hybrid": self.hybrid_cold_start_recommendations(
                user_id, n_recommendations
            ),
            "progressive": self.progressive_cold_start(
...
def run_cold_start_demo():
    ...
    for method_name, recommendations in methods_comparison.items():
        ...
        for item_id, score in recommendations:
            movie_title = (
                movies[movies["movieId"] == item_id]["title"].iloc[0]
                if not movies[movies["movieId"] == item_id].empty
                else f"Movie {item_id}"
            )
    ...
    print(f"\n Evaluation Results:")
    print(
        f"   User Cold Start Diversity: {evaluation_results['user_cold_start']['diversity_score']:.3f}"
    )
    print(
        f"   Item Cold Start Similarity: {evaluation_results['item_cold_start']['similarity_score']:.3f}"
    )
