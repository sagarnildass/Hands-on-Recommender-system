...
def implement_nmf(user_item_matrix, n_components=20, max_iter=200):
    ...
    nmf = NMF(n_components=n_components,
              ...
              l1_ratio=0.5)  # L1/L2 regularization ratio
...
def implement_svd_plus_plus(user_item_matrix, n_factors=20, learning_rate=0.01, n_epochs=20, reg_param=0.1):
    """
    ...
    for epoch in tqdm(range(n_epochs), desc="Training"):
        ...
        batch_size = 10000
        for batch_start in tqdm(range(0, len(ratings), batch_size), desc="Processing batches", leave=False):
            ...
            for idx in tqdm(range(len(batch_ratings)), desc="Processing ratings", leave=False):
                ...
                prediction = (mu + bu[user_idx] + bi[item_idx] + 
                ...
                U[user_idx] += learning_rate * (error * V[item_idx] - reg_param * U[user_idx])
                V[item_idx] += learning_rate * (error * (U[user_idx] + implicit_feedback) - reg_param * V[item_idx])
...
def evaluate_factorization_methods(user_item_matrix, movies, ratings):
    ...
    U_svd, V_svd, Y_svd, bu_svd, bi_svd, mu_svd = implement_svd_plus_plus(
        train_matrix, n_factors=10, learning_rate=0.01, n_epochs=10  # Reduced parameters
    )
    ...
    svd_rmse = calculate_test_rmse(test_matrix, U_svd, V_svd, method='svd_plus_plus', 
                                  Y=Y_svd, bu=bu_svd, bi=bi_svd, mu=mu_svd)
...
def calculate_test_rmse(test_matrix, user_factors, item_factors, method='nmf', **kwargs):
    """
    ...
    for user_idx, item_idx in tqdm(zip(test_user_indices, test_item_indices), desc="Processing test ratings", leave=False):
        if method == 'nmf':
            ...
            if Y is not None and bu is not None and bi is not None:
                ...
                if len(user_rated_items) > 0:
                    ...
                    pred = mu + bu[user_idx] + bi[item_idx] + np.dot(user_factors[user_idx] + implicit_feedback, item_factors[item_idx])
                else:
                    pred = mu + bu[user_idx] + bi[item_idx] + np.dot(user_factors[user_idx], item_factors[item_idx])
            else:
                pred = np.dot(user_factors[user_idx], item_factors[item_idx])
...
if __name__ == "__main__":
    ...
    print(f"🏆 Best method: {min(results, key=results.get)} (RMSE: {min(results.values()):.4f})")