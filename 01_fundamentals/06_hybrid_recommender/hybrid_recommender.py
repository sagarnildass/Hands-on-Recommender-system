...
class HybridRecommender:
    ...
    def __init__(
        self, popularity_weight=0.3, content_weight=0.3, collaborative_weight=0.4
    ):
    ...
    def get_recommendations(self, user_id, n_recommendations=10):
        ...
        if activity_level == "new_user":
            ...
            return self._get_hybrid_recommendations(
                user_id, n_recommendations, [0.7, 0.3, 0.0]
            )  # 70% popularity, 30% content

        elif activity_level == "early_personalization":
            return self._get_hybrid_recommendations(
                user_id, n_recommendations, [0.4, 0.5, 0.1]
            )  # 40% popularity, 50% content, 10% collaborative

        elif activity_level == "personalization":
            return self._get_hybrid_recommendations(
                user_id, n_recommendations, [0.2, 0.3, 0.5]
            )  # 20% popularity, 30% content, 50% collaborative

        else:  # advanced_personalization
            return self._get_hybrid_recommendations(
                user_id, n_recommendations, [0.1, 0.2, 0.7]
            )  # 10% popularity, 20% content, 70% collaborative
    ...
    def _get_hybrid_recommendations(self, user_id, n_recommendations, weights):
        ...
        for i, movie_id in enumerate(popularity_recs):
            final_scores[movie_id] = final_scores.get(
                movie_id, 0
            ) + popularity_weight * (1.0 / (i + 1))

        # Add content-based scores
        for i, movie_id in enumerate(content_recs):
            final_scores[movie_id] = final_scores.get(movie_id, 0) + content_weight * (
                1.0 / (i + 1)
            )
    ...
    def _get_content_recommendations(self, user_id, n_recommendations):
        ...
        movie_similarities = cosine_similarity([user_preferences], self.movie_features)[
            0
        ]
...
def test_hybrid_recommender(ratings, movies):
    ...
    for user_type, user_id in user_types.items():
        ...
        if user_id in ratings["userId"].values:
            ...
            print(
                f"   Activity level: {hybrid_recommender.get_user_activity_level(user_id)}"
            )
        else:
