...
def find_similar_users(user_item_matrix, target_user_id, n_similar=10, method='cosine'):
    ...
    """
    print(f"Finding users similar to User {target_user_id} using {method} similarity...")
    ...
    for user_id in user_item_matrix.index:
        if user_id != target_user_id:
            similarity = calculate_user_similarity(user_item_matrix, target_user_id, user_id, method)
...
def get_user_based_recommendations(user_item_matrix, target_user_id, movies, n_recommendations=10, method='cosine'):
    """
    Improved user-based collaborative filtering with better handling
    """
    # Find similar users
    similar_users = find_similar_users(user_item_matrix, target_user_id, n_similar=50, method=method)
