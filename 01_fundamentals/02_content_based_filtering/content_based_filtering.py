...
def load_and_prepare_data():
    ...
    movie_stats = (
        ...
        .agg({"rating": ["mean", "count"], "title": "first", "genres": "first"})
        .reset_index()
    )
...
def build_content_based_recommender(movie_features, movie_stats):
    ...
    similarity_df = pd.DataFrame(
        similarity_matrix, index=movie_stats["movieId"], columns=movie_stats["movieId"]
    )
...
def get_content_based_recommendations(
    movie_id, similarity_df, movie_stats, n_recommendations=10
):
    """
...
def get_user_recommendations(
    user_id, movie_ratings, similarity_df, movie_stats, n_recommendations=10
):
    """
...
def test_content_based_recommender(similarity_df, movie_stats, movie_ratings):
    ...
    popular_movies = movie_stats.nlargest(10, "rating_count")[
    ...
    for idx, movie in popular_movies.head(3).iterrows():
        ...
        for i, rec in recommendations.iterrows():
            ...
            print(
                f"      Avg Rating: {rec['avg_rating']:.2f} ({rec['rating_count']} ratings)"
            )
    ...
    genre_examples = {
        "Action": movie_stats[
            movie_stats["genres"].str.contains("Action", na=False)
        ].iloc[0],
        "Comedy": movie_stats[
            movie_stats["genres"].str.contains("Comedy", na=False)
        ].iloc[0],
        "Drama": movie_stats[
            movie_stats["genres"].str.contains("Drama", na=False)
        ].iloc[0],
    }
    ...
    for user_id in active_users.index[:2]:  # Test first 2 active users
        ...
        user_top_movies = movie_ratings[movie_ratings["userId"] == user_id].nlargest(
            3, "rating"
        )
        ...
        for _, movie in user_top_movies.iterrows():
            ...
            print(
                f"   - {movie_info['title']} ({movie['rating']} stars) - {movie_info['genres']}"
            )

        # Get recommendations
        recommendations = get_user_recommendations(
            user_id, movie_ratings, similarity_df, movie_stats, n_recommendations=5
        )
...
if __name__ == "__main__":
    ...
    print(
        f"📊 Dataset: {len(movie_stats)} movies with {movie_features.shape[1]} features"
    )
