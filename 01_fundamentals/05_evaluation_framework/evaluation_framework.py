...
class RecommendationEvaluator:
    ...
    def calculate_ndcg_at_k(self, actual_items, recommended_items, k=10):
        ...
        relevance_scores = [1 if item in actual_items else 0 for item in recommended_items[:k]]
...
def evaluate_ranking_quality(train_data, test_data, model, evaluator, k_values=[5, 10, 20]):
    """
    ...
    for user_id, user_test_data in test_by_user:
        ...
        relevant_items = user_test_data[user_test_data['rating'] >= 4]['movieId'].tolist()
        
        if len(relevant_items) == 0:
            continue
        
        # Get recommendations for this user
        recommended_items = model.get_recommendations(user_id, n_recommendations=max(k_values))
...
def plot_comparison_results(comparison_df):
    ...
    # RMSE comparison
    comparison_df['rmse'].plot(kind='bar', ax=axes[0,0], title='RMSE Comparison (Lower is Better)')
    axes[0,0].set_ylabel('RMSE')
    
    # Precision@10 comparison
    comparison_df['precision@10'].plot(kind='bar', ax=axes[0,1], title='Precision@10 Comparison (Higher is Better)')
    axes[0,1].set_ylabel('Precision@10')
    
    # Recall@10 comparison
    comparison_df['recall@10'].plot(kind='bar', ax=axes[1,0], title='Recall@10 Comparison (Higher is Better)')
    axes[1,1].set_ylabel('Recall@10')
    
    # NDCG@10 comparison
    comparison_df['ndcg@10'].plot(kind='bar', ax=axes[1,1], title='NDCG@10 Comparison (Higher is Better)')
...
class PopularityRecommender:
    ...
    def train(self, train_data, movies):
        ...
        self.popular_movies = movie_stats.sort_values('popularity_score', ascending=False)['movieId'].tolist()
...
class CollaborativeRecommender:
    ...
    def train(self, train_data, movies):
        ...
        self.movie_avg_ratings = self.user_item_matrix.mean().sort_values(ascending=False)
...
def run_comprehensive_evaluation(ratings, movies):
    ...
    for model_name, model in models.items():
        ...
        ranking_metrics = evaluate_ranking_quality(train_data, test_data, model, evaluator)
